...
import {
  ...
  Settings, LogOut, Menu, X, ShoppingBag, Bell,
} from 'lucide-react';
...
const Avatar = ({ name, size = 10 }) => (
  <div
    className={`w-${size} h-${size} rounded-full bg-primary-fixed/20 border border-primary-fixed/40
    ...
    <div className="px-5 py-4 border-b border-white/5 flex-shrink-0">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-primary-fixed uppercase tracking-wider truncate">
            {user?.membership?.plan?.name || 'Member'}
          </p>
        </div>
      </div>
    </div>
...
const ClientSidebar = () => {
  ...
  return (
    <>
      ...
      <aside
        className="hidden lg:flex flex-col fixed left-0 top-0 h-full w-[280px] z-40
                   bg-[#111] border-r border-white/5"
      >
        <SidebarBody
          onClose={null}
      ...
      {/* Logo LEFT  ·  [Bell + Burger + Logout] RIGHT */}
      <header
        className="lg:hidden fixed top-0 left-0 right-0 z-[45] h-16
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 bg-white/5 border border-white/10 rounded-lg touch-manipulation
