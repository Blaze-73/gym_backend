...
const AddMealModal = ({ onClose, onAdd }) => {
  const [form, setForm] = useState({ name: '', meal_type: 'breakfast', calories: '', protein_g: '', carbs_g: '', fats_g: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/nutrition/meals', {
        ...
        body: JSON.stringify({ ...form, log_date: today }),
      });
      const data = res.ok ? await res.json() : { ...form, id: Date.now() };
      onAdd(data);
    } catch {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-black font-headline uppercase">Log Meal</h3>
            ...
            <div className="grid grid-cols-4 gap-2">
              {['breakfast', 'lunch', 'dinner', 'snack'].map(t => (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-xs font-headline font-bold uppercase transition-all
                    ${form.meal_type === t ? 'bg-primary-fixed text-black' : 'bg-white/5 text-gray-400 hover:text-white border border-white/5'}`}
                >
                  {t}
                </button>
              ))}
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3 bg-white/5 border border-white/10 rounded-xl font-headline font-bold uppercase text-sm hover:bg-white/10 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            disabled={!form.name || !form.calories || saving}
            className="flex-1 py-3 bg-primary-fixed text-black rounded-xl font-headline font-black uppercase text-sm hover:scale-[1.02] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
...
const Nutrition = () => {
  ...
  const fetchNutritionData = async () => {
    try {
      const token = localStorage.getItem('token');
      const today = new Date().toISOString().split('T')[0];
      const res = await fetch(`/api/nutrition/${today}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        const data = await res.json();
        setMeals(data.meals || []);
        setTodayStats(s => ({
          ...s,
          calories: data.calories || 0, protein: data.protein_g || 0,
          carbs: data.carbs_g || 0, fats: data.fats_g || 0, water: data.water_ml || 0,
          targetCalories: data.target_calories || 2500, targetProtein: data.target_protein_g || 180,
          targetCarbs: data.target_carbs_g || 300, targetFats: data.target_fats_g || 80,
  ...
  const handleAddWater = async (amount) => {
    ...
    try {
      ...
      await fetch('/api/nutrition', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ log_date: today, water_ml: newWater }),
      });
    } catch { /* optimistic UI kept */ }
  };
  ...
  const handleMealAdded = (meal) => {
    ...
    setTodayStats(s => ({
      ...s,
      calories: s.calories + (parseInt(meal.calories) || 0),
      protein: s.protein + (parseFloat(meal.protein_g) || 0),
      carbs: s.carbs + (parseFloat(meal.carbs_g) || 0),
      fats: s.fats + (parseFloat(meal.fats_g) || 0),
    }));
  };

  const pct = (cur, target) => Math.min((cur / target) * 100, 100);
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-6 lg:p-8">
      ...
      <div className="bg-gradient-to-r from-primary-fixed/10 to-surface-container-high border border-primary-fixed/20 rounded-2xl p-6 mb-8">
        ...
        <div className="h-2.5 bg-white/5 rounded-full overflow-hidden mb-6">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${pct(todayStats.calories, todayStats.targetCalories)}%` }}
        ...
        <div className="lg:col-span-2">
          ...
          {meals.length === 0 ? (
            <div className="bg-surface-container-high border border-white/5 rounded-2xl p-10 text-center">
              ...
              <button
                onClick={() => setShowAddMeal(true)}
                className="mt-4 px-5 py-2.5 bg-primary-fixed text-black font-headline font-black uppercase text-sm rounded-full hover:scale-105 transition-transform"
              >
                Log First Meal
              </button>
            </div>
            ...
            <div className="space-y-3">
              {meals.map((meal, i) => (
                <motion.div
                  ...
                  className="bg-surface-container-high border border-white/5 rounded-xl p-4 flex items-center justify-between"
                >
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="px-2 py-0.5 bg-primary-fixed/10 text-primary-fixed text-[10px] font-headline font-bold uppercase rounded-full">
                        {meal.meal_type}
                      </span>
        ...
        <div className="space-y-6">
          <div className="bg-surface-container-high border border-white/5 rounded-2xl p-6">
            ...
            <div className="relative h-28 bg-white/5 rounded-xl mb-4 overflow-hidden">
              <motion.div
                initial={{ height: 0 }}
                animate={{ height: `${pct(todayStats.water, todayStats.targetWater)}%` }}
                transition={{ duration: 0.8 }}
                className="absolute bottom-0 left-0 right-0 bg-primary-fixed/25 rounded-xl"
              />
            ...
            <div className="grid grid-cols-3 gap-2">
              {[250, 500, 750].map(amt => (
                <button
                  ...
                  className="py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-headline font-bold
                             hover:bg-primary-fixed hover:text-black hover:border-primary-fixed transition-all"
                >
                  +{amt}ml
                </button>
              ))}
          ...
          {/* Supplements */}
          <div className="bg-surface-container-high border border-white/5 rounded-2xl p-6">
            <h3 className="font-headline font-bold uppercase text-sm mb-4">Supplement Protocol</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 bg-white/[0.03] rounded-xl border border-white/5">
                  <span className="text-sm text-gray-300 font-headline">{s.name}</span>
                  <span className="text-sm font-black text-primary-fixed">{s.dose}</span>
                </div>
              ))}
            </div>
          </div>
